class Personas:
    def __init__(self):
        self.personas = []
        #Jesus Garcia Florez y Carlos Daniel Ortiz
    ...
    def eliminar_persona(self):
        if not self.personas:
            ...
            for i, persona in enumerate(self.personas):
                print(f"{i+1}. {persona}")
            indice = int(input("porfavor Ingrese el número de la persona que deseas eliminar: "))
...
class Universidades:
    ...
    def crear_universidad(self):
        nombre = input("Ingresar el nombre de la universidad: ")
        self.universidades.append(nombre)
        print(f"Creaste la universidad: {nombre}")

    def listar_universidades(self):
        if not self.universidades:
            print("Esta Universidad no aparece registrada.")
        else:
    ...
    def eliminar_universidad(self):
        if not self.universidades:
            ...
            indice = int(input("Ingrese el número de la universidad que deseas eliminar: "))
            if 1 <= indice <= len(self.universidades):
                nombre_eliminado = self.universidades.pop(indice - 1)
                print(f"Usted Se ha eliminado la universidad: {nombre_eliminado}")
            else:
...
class Notas:
    ...
    def eliminar_nota(self):
        if not self.notas:
            print("La Nota que ingresaste No registrada.")
        else:
            print("Listado de notas:")
            for i, nota in enumerate(self.notas):
                print(f"{i+1}. {nota['titulo']}")
            indice = int(input("Ingrese el número de la nota que deseas eliminar eliminar: "))
            if 1 <= indice <= len(self.notas):
                nota_eliminada = self.notas.pop(indice - 1)
                print(f"Usted ha eliminado la nota: {nota_eliminada['titulo']}")
            else:
                print("Opción inválida.")
...
class Asignaturas:
    ...
    def crear_asignatura(self):
        ...
        print(f"Usted ha creado la asignatura: {nombre}")

    def listar_asignaturas(self):
        if not self.asignaturas:
            print("lo siento, Esta Asignatura no esta registrada.")
        else:
